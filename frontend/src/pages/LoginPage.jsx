      ...
      <div className="w-full max-w-[360px] bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        ...
        <div className="text-center mb-8">
          <div className="bg-violet-50 w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-4">
            <LogIn className="text-violet-600" size={24} />
          </div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Sign In</h2>
          <p className="text-slate-500 text-sm mt-1">Manage your finance today</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold hover:bg-black active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-sm shadow-sm"
          >
            {isLoading ? (
          ...
          <div className="text-center pt-2">
            <p className="text-sm text-slate-500 font-medium">
              New here?{' '}
              <Link to="/register" className="text-violet-600 font-bold hover:text-violet-700 underline-offset-4">
                Create Account
              </Link>
            </p>
          </div>
